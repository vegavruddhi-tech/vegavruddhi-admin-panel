...
export default function TLOverview({ firstLoad = true, onLoaded }) {
  ...
  const load = useCallback(async (force = false) => {
    ...
    try {
      const [tlRes, empRes] = await Promise.all([
        fetch(`${EMP_API}/tl/approved-list`).catch(() => ({ ok: false })),
        fetch(`${EMP_API}/auth/all-employees`).catch(() => ({ ok: false }))
      ]);
      const tlsList = tlRes.ok ? await tlRes.json().catch(() => ([])) : [];
      const usersList = empRes.ok ? await empRes.json().catch(() => ([])) : [];
  ...
  useEffect(() => {
    ...
    const cache = window.vv_cache || {};
    if (cache.universalVerify || cache.tlVerify) {
      setGlobalVerifyMap(cache.universalVerify || cache.tlVerify);
      return;
    }
    ...
    allForms.forEach(f => {
      ...
      map[key] = { status: s, points: f.verificationChecks?.points || 0, manualVerification: f.verificationChecks?.manualVerification };
    });
    setGlobalVerifyMap(map);
    if (!window.vv_cache) window.vv_cache = {};
    window.vv_cache.universalVerify = map;
  }, [allForms]); // Only run when forms data changes
